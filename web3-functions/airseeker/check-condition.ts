...
export const checkConditions = (
  ...
  if (!isFulfillmentDataFresh) {
    const log = logger.warn('Fulfillment data older than on-chain data. Skipping.');
    return [[log], false];
  }
  ...
  if (!isOnchainDataFresh) {
    const log = logger.info( 'On chain data timestamp older than heartbeat. Updating without condition check.');
    return [[log], true];
  } else {
    ...
    if (!shouldUpdate) {
      const log = ('Deviation threshold not reached. Skipping.');
      return [[log], false];
    }
  }
  const log = logger.info('Deviation threshold reached. Updating.');
  return [[log], true];
};
