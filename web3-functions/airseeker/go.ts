...
export interface GoAsyncOptions<E extends Error = Error> {
  ...
  onAttemptError?: (goRes: any ) => void; // Callback invoked after each failed attempt is completed. This callback does not fire for the last attempt or when a "totalTimeoutMs" is exceeded (these should be handled explicitly with the result of "go" call).
}
...
export const go = async <T, E extends Error>(
  ...
  const makeAttempts = async () => {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      const goRes = await attempt<T, E>(fn, currentAttemptTimeoutMs) as  GoResult<Awaited<T>, E>;
      ...
      lastFailedAttemptResult = goRes as  GoResultError<E>;
